...
def batch_search_buffer_collate_fn(data):
    return {
        'smiles': [o['smiles'] for o in data],
        'embs': torch.stack([torch.from_numpy(o['emb']) for o in data])
    }

def batch_to_device_search_buffer_collate_fn(data, device):
    return data['smiles'], data['embs'].to(device)

def batch_buffer_collate_fn(data, use_properties):
    ...
    return (
        [r_idx for r_idx, _, _ in tensors],
        [c_idx for _, c_idx, _ in tensors],
        properties,
    )

def batch_to_device_collate_fn(data, device):

    r_ids, c_ids, properties = data
...
def show_mols(mols, legends="new_indices", smiles_in=False, svg=False, sort_by_legend=False, max_mols=500,
              legend_float_decimals=4, mols_per_row=5, save_pth = None, target_mol=None, target_legend=None):
    ...
    if legends == 'new_indices':
        ...
        legends = [Descriptors.ExactMolWtx(m) for m in mols]
    ...
    if isinstance(legends, list):
        ...
        legends_str = [''] * len(mols)
        for k, v in legends.items():
            for i, o in enumerate(v):
                legends_str[i] = f'{legends_str[i]}{k}: {o:.{legend_float_decimals}f}\n\n'
        legends = legends_str
    if target_mol:
        target_mol = Chem.MolFromSmiles(target_mol) if isinstance(target_mol, str) else target_mol
        padding_mols = [target_mol] * (mols_per_row // 2)
        mols = padding_mols + [target_mol] + padding_mols + mols
        padding_legends = [''] * (mols_per_row // 2)
        legends = padding_legends + [target_legend] + padding_legends + legends
    img = Draw.MolsToGridImage(mols, maxMols=max_mols, subImgSize=(120, 150), legends=legends, molsPerRow=min(max_mols, mols_per_row),
