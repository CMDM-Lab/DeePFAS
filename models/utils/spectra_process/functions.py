...
def batch_spec_collate_fn(batch, resolution=1):
    ...
    intensity = [data[3][:, 1] for data in batch]
    peaks = [tokenzie_mz(data[3], resolution) for data in batch]

    ###########################

    # use neutral loss

    # losses = batch_cal_losses(zip(precursor_mz, m_z, intensity))
    # peaks = [tokenzie_mz(torch.stack([m + l[0], i + l[1]], dim=0), resolution) for m, i, l in zip(m_z, intensity, losses)]

    ###########################

    intensity = [peak[:, 1] for peak in peaks]
    m_z = [peak[:, 0].long() for peak in peaks]
    smiles_list = [data[0] for data in batch]
    randomized_smiles_list = [data[1] for data in batch]
    randomized_smiles_ids = smiles_batch_indices(randomized_smiles_list)
    canonical_smiles_ids = batch_add_eos_sos(smiles_list)
    tensors = [[r_idx, c_idx, m, i] for r_idx, c_idx, m, i in zip(randomized_smiles_ids, canonical_smiles_ids, m_z, intensity)]
    tensors.sort(key=lambda x: len(x[0]), reverse=True)

    return (
        [torch.tensor(r_idx) for r_idx, _, _, _ in tensors],
        [torch.tensor(c_idx) for _, c_idx, _, _ in tensors],
        [m.tolist() for _, _, m, _ in tensors],
        [i.tolist() for _, _, _, i in tensors]
    )
